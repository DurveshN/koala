...
- `koala.settings.dat` (serveur de bureau par défaut URL)
- `koala.global.dat` et `opencode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
