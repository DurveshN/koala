...
export default {
  id: "20260919163314_koala_industrial_audit_constraints",
  up(tx) {
    return Effect.gen(function* () {
      ...
      yield* tx.run(`
        CREATE TABLE \`__new_koala_tool_audit\` (
          ...
          CONSTRAINT "koala_tool_audit_lifecycle_check" CHECK(("state" = 'running' AND "time_finished" IS NULL AND "duration_ms" IS NULL AND "outcome_code" IS NULL AND "cancelled" = 0 AND "timed_out" = 0 AND "truncated" = 0 AND "error_code" IS NULL AND json_array_length("output_artifact_ids") = 0) OR ("state" = 'completed' AND "time_finished" IS NOT NULL AND "duration_ms" IS NOT NULL AND "outcome_code" IS NOT NULL)),
      ...
      `)
      yield* tx.run(
        `INSERT INTO \`__new_koala_tool_audit\`(\`id\`, \`state\`, \`tool_name\`, \`permission_class\`, \`session_id\`, \`message_id\`, \`tool_call_id\`, \`time_started\`, \`time_finished\`, \`duration_ms\`, \`outcome_code\`, \`cancelled\`, \`timed_out\`, \`truncated\`, \`engine_name\`, \`engine_version\`, \`contract_version\`, \`input_sha256\`, \`input_summary\`, \`source_artifact_ids\`, \`output_artifact_ids\`, \`sandbox_run_id\`, \`route_decision_id\`, \`error_code\`) SELECT \`id\`, \`state\`, \`tool_name\`, \`permission_class\`, \`session_id\`, \`message_id\`, \`tool_call_id\`, \`time_started\`, \`time_finished\`, \`duration_ms\`, \`outcome_code\`, \`cancelled\`, \`timed_out\`, \`truncated\`, \`engine_name\`, \`engine_version\`, \`contract_version\`, \`input_sha256\`, \`input_summary\`, \`source_artifact_ids\`, \`output_artifact_ids\`, \`sandbox_run_id\`, \`route_decision_id\`, \`error_code\` FROM \`koala_tool_audit\`;`,
      )
